...
export abstract class BaseCliProvider implements RhwpAiProvider {
  ...
  protected buildOperationPrompt(input: RhwpAiQuery): string {
    return [
      ...
      "- replace_document: { type, text }. Use this when the user asks to revise or rewrite the whole document.",
      ...
      "- save_document: { type }",
      "For Codex image generation, use the real image-generation tool/model such as gpt-image-2 when the CLI exposes it.",
      "For Antigravity image generation, use Nano Banana / Nano Banana Pro / Gemini image-generation capability when the CLI exposes it.",
      ...
      "Do not invent operation names. If the request is a full-document rewrite, use replace_document instead of rewrite_document or replace_all_text.",
